...
namespace Box.Sdk.Gen.Internal
{
    ...
    class DefaultNetworkClient : INetworkClient
    {
            ...
            if (options.ContentType == ContentTypes.MultipartFormData)
            {
                ...
                foreach (var part in options.MultipartData)
                {
                    HttpContent partContent = part.FileStream != null ?
                        new ReusableContent(part.FileStream) as HttpContent :
                        part.Data != null ?
        ...
        private static async System.Threading.Tasks.Task<T> AwaitWithCancellation<T>(System.Threading.Tasks.Task<T> task, CancellationToken cancellationToken)
        {
            ...
            var completedTask = await System.Threading.Tasks.Task.WhenAny(task, cancellationTask).ConfigureAwait(false);
        ...
        private static async Task<FetchResponse> ReadResponse(bool isStreamResponse, HttpResponseMessage response, int statusCode, string url, CancellationToken cancellationToken)
        {
            return isStreamResponse ?
                new FetchResponse(status: statusCode, headers: response.Headers.ToDictionary(x => x.Key, x => x.Value.First())) { Url = url, Content = await response.Content.ReadAsStreamAsync().ConfigureAwait(false) } :
                new FetchResponse(status: statusCode, headers: response.Headers.ToDictionary(x => x.Key, x => x.Value.First())) { Url = url, Data = JsonUtils.JsonToSerializedData(await response.Content.ReadAsStringAsync().ConfigureAwait(false)) };
        }
    }
}
