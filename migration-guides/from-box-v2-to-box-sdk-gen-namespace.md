# Migration guide: migrate from `Box.V2` to `Box.Sdk.Gen` namespace
...
- v5: ships only `Box.V2` namespace
- v6: ships both `Box.V2` and `Box.Sdk.Gen` namespaces (side-by-side)
- v10+: ships only `Box.Sdk.Gen` namespace

This document focuses on helping you migrate code from the legacy `Box.V2` namespace to the generated `Box.Sdk.Gen` namespace. Many APIs were redesigned for consistency and modern C# patterns, so this guide calls out how to adopt the new shapes safely and incrementally.

Supported migration paths:

- v5 → v6: adopt `Box.Sdk.Gen` gradually while legacy code still uses `Box.V2`
- v5 → v10+: migrate directly to `Box.Sdk.Gen` only
- v6 (within the same app): move usage from `Box.V2` to `Box.Sdk.Gen` file-by-file
...
Recommended direction: use `Box.Sdk.Gen` going forward. Because it is auto-generated from the Box OpenAPI specification, it:
...
- Teams with existing code using `Box.V2` who want to start calling `Box.Sdk.Gen` APIs.
- Teams already on an SDK release that provides both namespaces and want to transition usage from `Box.V2` to `Box.Sdk.Gen` within the same app.

## Migration at a glance

- Replace `using Box.V2` with `using Box.Sdk.Gen` in files you’re migrating.
...
The `Box.Sdk.Gen` namespace now supports [nullable reference types](https://learn.microsoft.com/en-us/dotnet/csharp/nullable-references) whenever possible. This allows to perform null state analyis and detect many problems even before the code is run.
...
In the `Box.V2` namespace all objects exposed setters and an empty constructor. It was possible to create an object without setting some of the fields required by the API. This would result in the API error (usually Bad Request) saying that the request was missing some requeired fields. Some of the functions validated if the request fields are set properly, but even then, the lack of required fields could not be detected when call to such function was executed.

The `Box.Sdk.Gen` namespace is designed to minimize problems with object creation and "force" the creation of objects in the correct state by utilizng constructors capabilities. Let's look at the following example
...
The `Box.V2` namespace contained multiple smaller namespaces which greatly reduced the discoverability of new features. The namespaces were also grouped based on their physical location rather than purpose they serve. Developers had to navigate through numerous namespaces to find the classes and methods they needed, which could be overwhelming and confusing, especially for newcomers.

To simplify it, our new `Box.Sdk.Gen` namespace, provides only 4 main namespaces:
...
The `Box.V2` namespace used [C# enumeration types](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/enum) when API exposed model with limited number of possible value for `String`.
...
To overcome these issues the `Box.Sdk.Gen` namespace wraps enum types in safe, custom `StringEnum` structure.
...
The `Box.V2` namespace used approach with common ancestor, by returning base object (in this case `BoxEntity`) that needed to be explicitly cast to the desired type.
...
Authentication is a crucial aspect of any SDK. Let's delve into the authentication methods supported by both namespaces of the SDKs and understand the enhancements in the new namespace:
...
The new namespace of the SDK offers a more streamlined approach:
...
The `Box.Sdk.Gen` namespace provides a more organized approach:
...
For those who prefer manual configurations, both namespaces offer flexibility in this regard:
...
The `Box.Sdk.Gen` namespace introduces a more structured approach:
...
The new namespace of the SDK makes this method more organised:
...
The new namespace of the SDK offers a more organized structure:
...
The new namespace of the SDK streamlines the process:
...
The new namespace of the SDK provides more flexibility:
...
The new namespace of the SDK simplifies the process:
...
Token management is crucial for maintaining secure sessions. The new namespace of the SDK offers enhanced flexibility:

**Old (`Box.V2`):**

```c#
// there are no good ways to implement token storage in the Box.V2 namespace of the sdk
...
The new namespace of the SDK allows developers to define custom classes for token storage:
...
In the `Box.V2` namespace you could call client constructor with `asUser` parameter to create a new client to impersonate the provided user.
...
In the `Box.Sdk.Gen` namespace the method was renamed to `WithAsUserHeader(string userId)`
...
In the `Box.V2` namespace you could specify the custom base URLs, which will be used for API calls made by setting
...
In the `Box.Sdk.Gen` namespace this functionality has been implemented as part of the `BoxClient` class.
...
In the `Box.V2` namespace, you could use the `UploadUsingSessionAsync` method of the `FilesManager` class to upload a large file.
...
In the `Box.Sdk.Gen` namespace, the equivalent method is `ChunkedUploads.UploadBigFileAsync()`. It accepts a `Stream` object
