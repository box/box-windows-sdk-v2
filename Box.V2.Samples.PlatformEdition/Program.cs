...
namespace Box.V2.Samples.PlatformEdition
{
    class Program
    {
        ...
        static readonly string DEVELOPER_TOKEN = "AHIhmE8KrQgIb2a75futuWSR9Ft8foge";
        ...
        static void Main(string[] args)
        {
            ...
            //var adminToken = BoxPlatformEdition.EnterpriseToken(ENTERPRISE_ID, CLIENT_ID, CLIENT_SECRET, privateKey, JWT_PRIVATE_KEY_PASSWORD);
        ...
        static async Task MainAsync()
        {
            ...
            var downloadStream = await adminClient.FilesManager.DownloadStreamAsync("36828031270", timeout: TimeSpan.FromMinutes(10));
            Console.WriteLine("Writing to disk");
            using (FileStream output = File.OpenWrite("test.mp4"))
            {
                await downloadStream.CopyToAsync(output);
            }

            Console.WriteLine("Finished writing to disk");
            Console.WriteLine("Finished download: {0}", DateTime.Now - start);
