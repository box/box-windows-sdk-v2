...
namespace Box.V2.Test.Integration
{
    ...
    public class BoxFoldersManagerTestIntegration : BoxResourceManagerTestIntegration
    {
        ...
        public async Task RestoreFolder_ValidResponse()
        {
            ...
            _ = await Client.FoldersManager.RestoreTrashedFolderAsync(folderRequest);
        }

        [TestMethod]
        public async Task GetFolder_LiveSession_ValidResponse()
        {
            await AssertFolderContents(Client);
        }
        ...
        public async Task GetFolderInformation_Fields_ValidResponse()
        {
            ...
            var folder = await Client.FoldersManager.GetInformationAsync(FolderId, fields: new List<string> { "allowed_shared_link_access_levels", "can_non_owners_invite", "is_externally_owned" });
        ...
        public async Task GetFolderInformation_Fields_Metadata_ValidResponse()
        {
            ...
            var folder = await Client.FoldersManager.GetInformationAsync(FolderId, fields: new List<string> { "metadata.enterprise_440385.testtemplate" });
            ...
            folder = await Client.FoldersManager.GetInformationAsync(FolderId, fields: new List<string> { "metadata.enterprise.testtemplate" });
        ...
        public async Task UpdateFolderInformation_ValidRequest_Timeout()
        {
            ...
            _ = await Client.FoldersManager.UpdateInformationAsync(folderRequest: folderReq, timeout: timeout);
        }
        ...
        {
            var results = await Client.FoldersManager.GetTrashItemsAsync(10);
        ...
        public async Task Watermark_Folders_CRUD()
        {
            ...
            var folder = await Client.FoldersManager.GetInformationAsync(folderId, mylist);

            Assert.IsFalse(folder.WatermarkInfo.IsWatermarked);

            var watermark = await Client.FoldersManager.ApplyWatermarkAsync(folderId);
            Assert.IsNotNull(watermark, "Failed to apply watermark to folder");

            folder = await Client.FoldersManager.GetInformationAsync(folderId, mylist);
            Assert.IsTrue(folder.WatermarkInfo.IsWatermarked);

            var fetchedWatermark = await Client.FoldersManager.GetWatermarkAsync(folderId);
            Assert.IsNotNull(fetchedWatermark, "Failed to fetch watermark of folder");

            var result = await Client.FoldersManager.RemoveWatermarkAsync(folderId);
        ...
        public async Task FolderWorkflow_LiveSession_ValidResponse()
        {
            ...
            BoxFolder fi = await Client.FoldersManager.GetInformationAsync(f.Id);
            ...
            BoxFolder fsl = await Client.FoldersManager.CreateSharedLinkAsync(f.Id, sharedLinkReq);
            ...
            var newTestname = GetUniqueName();
            var updateReq = new BoxFolderRequest()
            {
                Id = f.Id,
                Name = newTestname,
            ...
            BoxFolder uf = await Client.FoldersManager.UpdateInformationAsync(updateReq);

            Assert.AreEqual(newTestname, uf.Name, "New folder name is not correct");

            // Test Copy Folder
            var copyTestName = GetUniqueName();
            ...
            BoxFolder f2 = await Client.FoldersManager.CopyAsync(copyReq);

            Assert.AreEqual(copyTestName, f2.Name, "Copied file does not have correct name");

            //Clean up - Delete Test Folders
            await Client.FoldersManager.DeleteAsync(f.Id, true);
            await Client.FoldersManager.DeleteAsync(f2.Id, true);
        }
        ...
        public async Task FolderSharedLink_CreateAndDelete_ValidResponse()
        {
            ...
            BoxFolder sl = await Client.FoldersManager.CreateSharedLinkAsync(f.Id, sharedLinkReq);
            ...
            sl = await Client.FoldersManager.CreateSharedLinkAsync(f.Id, sharedLinkReq);
            Assert.AreEqual(sl.Id, f.Id);
            Assert.IsNotNull(sl.SharedLink);
            Assert.AreEqual(sl.SharedLink.Access, BoxSharedLinkAccessType.open);
            Assert.IsNotNull(sl.SharedLink.Permissions);
            ...
            sl = await Client.FoldersManager.DeleteSharedLinkAsync(f.Id);
            Assert.AreEqual(sl.Id, f.Id);
