...
namespace Box.Sdk.Gen.Tests.Integration {
    ...
    public class MetadataTaxonomiesManagerTests {
        ...
        public async System.Threading.Tasks.Task TestMetadataTaxonomiesNodes() {
            ...
            Assert.IsTrue(NullableUtils.Unwrap(metadataTemplate.Fields)[0].DisplayName == fieldDisplayName);
            Assert.IsTrue(NullableUtils.Unwrap(metadataTemplate.Fields)[0].Hidden == false);
            Assert.IsTrue(NullableUtils.Unwrap(metadataTemplate.Fields)[0].Id != "");
            Assert.IsTrue(NullableUtils.Unwrap(metadataTemplate.Fields)[0].Key == fieldKey);
            Assert.IsTrue(NullableUtils.Unwrap(metadataTemplate.Fields)[0].NamespaceParam == namespaceParam);
            Assert.IsTrue(NullableUtils.Unwrap(NullableUtils.Unwrap(metadataTemplate.Fields)[0].OptionsRules).MultiSelect == true);
            Assert.IsTrue(NullableUtils.Unwrap(NullableUtils.Unwrap(NullableUtils.Unwrap(metadataTemplate.Fields)[0].OptionsRules).SelectableLevels)[0] == 1);
            Assert.IsTrue(NullableUtils.Unwrap(metadataTemplate.Fields)[0].TaxonomyId != "");
            Assert.IsTrue(NullableUtils.Unwrap(metadataTemplate.Fields)[0].TaxonomyKey == taxonomyKey);
            Assert.IsTrue(StringUtils.ToStringRepresentation(NullableUtils.Unwrap(metadataTemplate.Fields)[0].Type) == "taxonomy");
            MetadataTaxonomyNodes options = await client.MetadataTaxonomies.GetMetadataTemplateFieldOptionsAsync(namespaceParam: namespaceParam, templateKey: metadataTemplateKey, fieldKey: fieldKey);
