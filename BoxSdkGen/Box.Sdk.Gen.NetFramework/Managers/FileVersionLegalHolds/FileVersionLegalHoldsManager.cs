...
namespace Box.Sdk.Gen.Managers {
    public class FileVersionLegalHoldsManager {
        ...
        /// </param>
        public async System.Threading.Tasks.Task<FileVersionLegalHold> GetFileVersionLegalHoldByIdAsync(string fileVersionLegalHoldId, GetFileVersionLegalHoldByIdHeaders headers = default, System.Threading.CancellationToken cancellationToken = default) {
            headers = headers ?? new GetFileVersionLegalHoldByIdHeaders();
            Dictionary<string, string> headersMap = Utils.PrepareParams(map: DictionaryUtils.MergeDictionaries(new Dictionary<string, string>() {  }, headers.ExtraHeaders));
            FetchResponse response = await this.NetworkSession.NetworkClient.FetchAsync(options: new FetchOptions(url: string.Concat(this.NetworkSession.BaseUrls.BaseUrl, "/2.0/file_version_legal_holds/", NullableUtils.Unwrap(StringUtils.ToStringRepresentation(fileVersionLegalHoldId))), method: "GET", responseFormat: Box.Sdk.Gen.ResponseFormat.Json) { Headers = headersMap, Auth = this.Auth, NetworkSession = this.NetworkSession, CancellationToken = cancellationToken }).ConfigureAwait(false);
        ...
        /// Token used for request cancellation.
        /// </param>
        public async System.Threading.Tasks.Task<FileVersionLegalHolds> GetFileVersionLegalHoldsAsync(GetFileVersionLegalHoldsQueryParams queryParams, GetFileVersionLegalHoldsHeaders headers = default, System.Threading.CancellationToken cancellationToken = default) {
