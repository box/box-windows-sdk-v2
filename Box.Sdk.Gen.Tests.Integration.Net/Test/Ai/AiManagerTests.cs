...
namespace Box.Sdk.Gen.Tests.Integration {
    ...
    public class AiManagerTests {
        ...
        public async System.Threading.Tasks.Task TestAiExtractStructuredWithFields() {
            ...
            Assert.IsTrue(response.Reference != null);
            Assert.IsTrue(response.AiAgentInfo != null);
            ...
            Assert.IsTrue(NullableUtils.Unwrap(StringUtils.ToStringRepresentation(response.Answer["address"])).Contains("Redwood City"));
            Assert.IsTrue(NullableUtils.Unwrap(StringUtils.ToStringRepresentation(response.Answer["address"])).Contains("CA"));
            Assert.IsTrue(NullableUtils.Unwrap(StringUtils.ToStringRepresentation(response.Answer["address"])).Contains("94063"));
            Assert.IsTrue(NullableUtils.Unwrap(StringUtils.ToStringRepresentation(response.Answer["work_history"])).Contains("Box"));
            Assert.IsTrue(response.CompletionReason == "done");
            await client.Files.DeleteFileByIdAsync(fileId: file.Id);
        }
